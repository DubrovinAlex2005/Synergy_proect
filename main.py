...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(
            self, columns=("ID", "name", "tel", "email", "payment"), height=45, show="headings"
        )
    ...
    def update_records(self, name, tel, email, payment):
        self.db.cursor.execute(
            """UPDATE db SET name=?, tel=?, email=?, payment=? WHERE id=?""",
    ...
    def init_child(self):
        ...
        self.btn_ok.bind(
            ...
            lambda event: self.view.records(
                self.entry_name.get(), self.entry_email.get(), self.entry_tel.get(), self.entry_payment.get()
            ),
        )
...
class DB:
    ...
    def insert_data(self, name, tel, email, payment):
        self.cursor.execute(
            """INSERT INTO db(name, tel, email, payment) VALUES(?, ?, ?, ?)""", (name, tel, email, payment)
        )
        self.conn.commit()
